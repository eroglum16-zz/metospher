...
(function($) {
    ...
    function isValidImgUrl(url, callback) {
        var img = new Image();
        img.onerror = function() { callback(url, false, "Bu formata izin verilmiyor."); };
        img.onload =  function() {
            ...
            if ($.inArray(fileExt, options.allowedFormats) > -1) {
                ...
                callback(url, false, "Dosya boyutu uygun değil.");
            }
        };
        img.src = url;
    }
    ...
    function selectImgFile($selectBtn) {
        ...
        if ($.inArray(fileExt, options.allowedFormats) > -1) {
            ...
            $selectBtn.find("span").text("Değiştir");
            $fileTab.find(".img-remove-btn").css("display", "inline-block");
        }
        else {
            $fileTab.prepend(getHtmlErrorMsg("Format uygun değil."));
        }
    }
    ...
    function removeImgFile($removeBtn) {
        ...
        $fileTab.find(".alert").remove();
        $fileTab.find(".img-select-btn span").text("Fotoğraf seçin...");
